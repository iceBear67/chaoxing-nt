...
class Renderer {
  ...
  drawFrame(
    imageData = { header, yUint8Array, uUint8Array, vUint8Array, yStride }
  ) {
    if (!this.ready) {
      this.ready = true;
    ...
    if (!imageData.yStride) {
      imageData.yStride = width;
    }
    this.updateCanvas({
      width,
      height,
    ...
    let y = YUVBuffer.lumaPlane(
      format,
      imageData.yUint8Array,
      imageData.yStride
    );
    let u = YUVBuffer.chromaPlane(
      format,
      imageData.uUint8Array,
      imageData.yStride / 2
    );
    let v = YUVBuffer.chromaPlane(
      format,
      imageData.vUint8Array,
      imageData.yStride / 2
    );
